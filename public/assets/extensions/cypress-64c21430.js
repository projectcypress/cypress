...
function filterVisibleMeasureTabs(searchbox, measure_tabs_response) {
  ...
  $.each(measure_tabs, function () {
    ...
    if (current_tab_name in measure_tabs_response) {
      ...
      $(this)
        .contents("a")
        .text(measure_tabs_response[current_tab_name])
        .append($cache);
    } else {
      $(this).hide();
    }
...
function UpdateGroupSelections(event) {
  ...
  var $groupChecks = $(
    ".measure-group .measure-checkbox[data-category=" + measure_category + "]",
  );

  var groupIsSelected = !$groupChecks.filter(":not(:checked)").length; // true if none are unchecked

  $(".measure-group-all[id=" + measure_category + "]").prop(
    "checked",
    groupIsSelected,
  );
  ...
  $("#measure_tabs .ui-tabs-nav")
    .find("[href*=" + measure_category + "] .selected-number")
    .html(function () {
      if (number_checked > 0) {
        return (
          number_checked +
  ...
  $(".select-measures .card-title .selected-number").html(function () {
    if ($(".measure-group .measure-checkbox:checked").length > 0) {
      return (
        $(".measure-group .measure-checkbox:checked").length +
        '<i aria-hidden="true" class="fas fa-fw fa-check"></i>'
      );
    } else {
      return "(0)";
...
function HookupProductSearch() {
  ...
  var current_search = encodeURIComponent(
    $("#product_search_measures")
      .val()
      .replace(/[!'()*]/g, ""),
  );
...
ready_run_on_refresh_bundle = function () {
  ...
  $(".measure-group-all").on("change", function () {
    $(this)
      ...
      .find(
        ".measure-checkbox[data-category=" +
          escapeCSS($(this).attr("id")) +
          "]",
      )
...
export function initializeAdmin() {
  ...
  $(".activity-paginate").click(function (event) {
    Turbo.ProgressBar.start();
    Turbo.ProgressBar.advanceTo(25);
  });
}
