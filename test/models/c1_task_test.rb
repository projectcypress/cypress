...
class C1TaskTest < ActiveSupport::TestCase
  ...
  def test_should_be_able_to_test_a_good_archive_of_qrda_files
    ...
    task = @product_test.tasks.create({}, C1Task)
    zip = File.new(File.join(Rails.root, 'test/fixtures/qrda/cat_I/ep_qrda_test_good.zip'))
  ...
  def test_should_be_able_to_test_a_good_archive_of_shifted_qrda_files_for_shifted_test
    ...
    task = @product_test.tasks.create({}, C1Task)
    @product_test.product.c1_test = true
    zip = File.new(File.join(Rails.root, 'test/fixtures/qrda/cat_I/ep_qrda_test_good_shift.zip'))
    perform_enqueued_jobs do
      te = task.execute(zip, User.first)
      te.reload
  ...
  def test_should_be_able_to_test_a_good_archive_of_augmented_qrda_files
    task = @product_test.tasks.create({}, C1Task)
    @product_test.product.c1_test = true
    zip = File.new(File.join(Rails.root, 'test/fixtures/qrda/cat_I/ep_qrda_test_good_aug.zip'))
    perform_enqueued_jobs do
      te = task.execute(zip, User.first)
      te.reload
  ...
  def test_should_be_able_to_tell_when_wrong_number_of_documents_are_supplied_in_archive
    task = @product_test.tasks.create({}, C1Task)
    @product_test.product.c1_test = true
    zip = File.new(File.join(Rails.root, 'test/fixtures/qrda/cat_I/ep_qrda_test_too_many_files.zip'))
    perform_enqueued_jobs do
      te = task.execute(zip, User.first)
      te.reload
  ...
  def test_should_be_able_to_tell_when_wrong_names_are_provided_in_documents
    task = @product_test.tasks.create({}, C1Task)
    @product_test.product.c1_test = true
    zip = File.new(File.join(Rails.root, 'test/fixtures/qrda/cat_I/ep_qrda_test_wrong_names.zip'))
    perform_enqueued_jobs do
      te = task.execute(zip, User.first)
      te.reload
  ...
  def test_should_return_warning_when_incorrect_templates_in_c1_without_c3
    task = @product_test.tasks.create({}, C1Task)
    @product_test.product.c1_test = true
    zip = File.new(File.join(Rails.root, 'test/fixtures/qrda/cat_I/ep_qrda_test_wrong_templates.zip'))
    perform_enqueued_jobs do
      te = task.execute(zip, User.first)
      te.reload
  ...
  def test_should_be_able_to_tell_when_potentialy_too_much_data_is_in_documents
    ...
    zip = File.new(File.join(Rails.root, 'test/fixtures/qrda/cat_I/ep_qrda_test_too_much_data.zip'))
    perform_enqueued_jobs do
  ...
  def test_should_be_able_to_tell_when_calculation_errors_exist
    task = @product_test.tasks.create({}, C1Task)
    @product_test.product.c1_test = true
    zip = File.new(File.join(Rails.root, 'test/fixtures/qrda/cat_I/ep_qrda_bad_calculation.zip'))
    perform_enqueued_jobs do
      te = task.execute(zip, User.first)
      te.reload
  ...
  def test_should_be_able_to_tell_when_calculation_errors_exist_in_augmented_files
    task = @product_test.tasks.create({}, C1Task)
    @product_test.product.c1_test = true
    zip = File.new(File.join(Rails.root, 'test/fixtures/qrda/cat_I/ep_qrda_bad_calculation_aug.zip'))
    perform_enqueued_jobs do
      te = task.execute(zip, User.first)
      te.reload
      # 1 NUMER and 1 DENOM population
