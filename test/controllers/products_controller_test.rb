...
class ProductsControllerTest < ActionController::TestCase
  ...
  setup do
    ...
    @first_product = FactoryGirl.create(:product_static_bundle)
  ...
  test 'should create' do
    ...
    for_each_logged_in_user([ADMIN, ATL, OWNER]) do
      post :create, vendor_id: @vendor.id, product: { name: "test_product_#{rand}", c1_test: true, bundle_id: @bundle_id,
  ...
  test 'should not get zip file of patients if invalid product_id' do
    for_each_logged_in_user([ADMIN, ATL, OWNER, VENDOR]) do
      get :patients, :vendor_id => @first_product.vendor.id, :id => 'bad_id'
      assert_response 404, 'response should be Unprocessable Entity on report if bad id'
      assert_equal 'Not Found', response.message
    end
  end
  ...
  test 'should create checklist test on product create when c1 is selected' do
    for_each_logged_in_user([ADMIN, ATL, OWNER]) do
      ...
      post :create, :format => :json, :vendor_id => @vendor.id, :product => { :name => product_name, :c1_test => true,
                                                                              :measure_ids => ['BE65090C-EB1F-11E7-8C3F-9A214CF093AE'],
                                                                              :bundle_id => @bundle_id }
  ...
  test 'should not post create with json request with no name' do
    ...
    for_each_logged_in_user([ADMIN, ATL, OWNER]) do
      post :create, :format => :json, :vendor_id => vendor.id, :product => { :c1_test => true, :bundle_id => @bundle_id,
                                                                             :measure_ids => ['BE65090C-EB1F-11E7-8C3F-9A214CF093AE'] }
  ...
  test 'should strip trailing whitespace from name on create' do
    ...
    for_each_logged_in_user([ADMIN, ATL, OWNER]) do
      ...
      post :create, :vendor_id => @vendor.id, :product => { :name => "#{name} ", :c1_test => true, :bundle_id => @bundle_id,
                                                            :measure_ids => ['BE65090C-EB1F-11E7-8C3F-9A214CF093AE'] }
