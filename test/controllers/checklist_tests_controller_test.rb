...
class ChecklistTestsControllerTest < ActionController::TestCase
  ...
  test 'should be able to view measure' do
    ...
    product = @vendor.products.create!(name: "my product #{rand}", c1_test: true, bundle_id: @bundle.id, measure_ids:)
    product.product_tests.build({ name: "my measure test #{rand}", measure_ids: }, MeasureTest)
    product.save!
    checklist_test = product.product_tests.create!({ name: "my measure test #{rand}", measure_ids: }, ChecklistTest)
