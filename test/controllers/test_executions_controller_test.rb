...
class TestExecutionsControllerTest < ActionController::TestCase
  ...
  def create_execution_with_task_type(task_type)
    ...
    measure_ids = ['BE65090C-EB1F-11E7-8C3F-9A214CF093AE']
    product = @vendor.products.build(name: "product #{rand}", bundle_id: @bundle_id, measure_ids: measure_ids, c1_test: c1, c2_test: c2, c4_test: c4)
    product.save!
    ...
    Artifact.create!(test_execution: execution, file: File.new(Rails.root.join('test', 'fixtures', 'qrda', file_name)))
