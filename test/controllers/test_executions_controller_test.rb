...
class TestExecutionsControllerTest < ActionController::TestCase
  ...
  test 'should post create' do
    ...
    zipfile = File.new(File.join(Rails.root, 'test/fixtures/qrda/cat_III/ep_test_qrda_cat3_good.xml'))
  ...
  def create_execution_with_task_type(task_type)
    ...
    file = File.new(File.join(Rails.root, "test/fixtures/qrda/#{file_name}"))
  ...
  def zip_upload
    zipfile = File.new(File.join(Rails.root, 'test/fixtures/qrda/cat_I/ep_qrda_test_good.zip'))
    Rack::Test::UploadedFile.new(zipfile, 'application/zip')
  end

  def xml_upload
    xmlfile = File.new(File.join(Rails.root, 'test/fixtures/qrda/cat_III/ep_test_qrda_cat3_good.xml'))
