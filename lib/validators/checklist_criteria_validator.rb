  ...
  class ChecklistCriteriaValidator < QrdaFileValidator
    ...
    def validate(file, _options = {})
      ...
      @criteria_list.each do |criteria|
        ...
        if validate_criteria(criteria, patient)
    ...
    def validate_criteria(checked_criteria, patient)
      ...
      if (['2.16.840.1.113883.3.560.1.406', '2.16.840.1.113883.3.560.1.403', '2.16.840.1.113883.3.560.1.402'] & oids).present?
        ...
        data_elements = patient.get_data_elements.where(hqmfOid: { '$in' => oids })
        attribute = checked_criteria.attribute_index ? sdc['attributes'][checked_criteria.attribute_index]['attribute_name'] : nil
        return data_elements_that_meet_criteria(data_elements, checked_criteria, attribute).length.positive?
      end
    end
    ...
    def confirm_qdm_type_have_contents(attribute, checked_criteria)
      ...
      return verify_id_attribute(attribute) if attribute[:_type].nil? && attribute['_type'].nil?
