...
#  3) Install Git
#  4) Install RVM and Ruby 2.2.3
#  5) Install MongoDB
#  6) Install libxml2 2.9 from source
#  7) Install Nokogiri
...
# Date:   23 Sept 2014--26 Feb 2016
##############################################################################

# Variables that determine the versions of components we will install
cypress_tag="v3.0.0alpha6"
install_ruby_ver="2.2.3"
install_bundler_ver="1.11"
# install_libxml_ver="2.8.0"

# install_nokogiri_ver="1.6.7.2"
cypress_bundle_ver="2.9.9"

install_apache_ver="2.4.*"
install_passenger_ver="5.0.25"

install_mongodb_ver="3.0.9"
...
for p in build-essential openssl libssl-dev libreadline6 libreadline6-dev curl zlib1g zlib1g-dev libyaml-dev libsqlite3-dev sqlite3 libxml2-dev libxslt-dev autoconf libc6-dev ncurses-dev automake libtool bison subversion pkg-config libgmp3-dev nodejs; do
  echo -n "      $p: "
  install_pkg "$p"
done
...
if [ $? -eq 0 ]; then
  ...
  gpg --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 409B6B1796C275462A1703113804BB82D39DC0E3 &> /dev/null
...
grep -q "deb[[:space:]]\+.*repo\.mongodb\.org" /etc/apt/sources.list
if [ $? -eq 0 ]; then
  # it's there, but could be commented out
  grep -q "^#[[:space:]]*deb[[:space:]]\+.*repo\.mongodb\.org" /etc/apt/sources.list
  if [ $? -eq 0 ]; then
    # Uncomment it
    uncomment_line "^#[[:space:]]*" "deb[[:space:]]\+.*repo\.mongodb\.org" /etc/apt/sources.list
    success "enabled"
    update_sources=1
  else
...
output=`sudo service mongod status`
...
##########
# echo -n "Install libxml2 verison $install_libxml_ver: "
...
# install_gem "nokogiri" "$install_nokogiri_ver"
# echo
...
su - -c "source /usr/local/rvm/scripts/rvm; cd cypress; bundle install --without development test" cypress
success_or_fail $? "done" "failed"
...
devise_key=`cat /dev/urandom | env LC_CTYPE=c tr -dc a-e0-9 | fold -w 64 | head -n 1`
...
SetEnv DEVISE_SECRET_KEY $devise_key
CYPRESS_MOD_END
ln -s /etc/apache2/conf-available/cypress /etc/apache2/conf-enabled/cypress.conf
success "done"
# install variable into the 'cypress' user too
cat << BASHRC_END > /home/cypress/.bashrc
  export DEVISE_SECRET_KEY=${devise_key}
BASHRC_END
...
mkdir -p ./tmp/delayed_pids
RAILS_ENV=production DEVISE_SECRET_KEY=${devise_key} ./bin/delayed_job start -n 3
...
echo "Defaults   env_keep += DEVISE_SECRET_KEY\n" >> /etc/sudoers
