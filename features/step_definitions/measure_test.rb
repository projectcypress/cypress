...
And(/^the user uploads a CAT 1 zip file$/) do
  zip_path = File.join(Rails.root, 'test/fixtures/qrda/cat_I/ep_qrda_test_good.zip')
  page.attach_file('results', zip_path, visible: false)
  page.find('#submit-upload').click
end

And(/^the user uploads a CAT 3 XML file$/) do
  xml_path = File.join(Rails.root, 'test/fixtures/qrda/cat_III/ep_test_qrda_cat3_good.xml')
